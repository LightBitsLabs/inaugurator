...
def get_fio():
    try:
        r = sh.run('fio --name=randwrite --ioengine=libaio --iodepth=1 --rw=randwrite --bs=4k --direct=1 --numjobs=1 --size=10m --time_base=1 --runtime=60 --group_reporting --filename=/destRoot/fio_test --output-format=json')
        return json.loads(r)
    except Exception as e:
        return []

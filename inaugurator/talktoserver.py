...
class TalkToServerSpooler(threading.Thread):
    ...
    def _connect(self, amqpURL):
        ...
        self._channel.exchange_declare(exchange=self._statusExchange, type='fanout', auto_delete=True)
        logging.info("Declaring a RabbitMQ exchange %(exchange)s...", dict(exchange=self._labelExchange))
        self._channel.exchange_declare(exchange=self._labelExchange, type='fanout', auto_delete=True)

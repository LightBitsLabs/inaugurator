...
class Mount:
    ...
    def mountBootInsideRoot(self):
        sh.run("/usr/sbin/busybox mount -t ext4 %s %s/boot" % (self._bootPartitionLegacy, self._ROOT_MOUNT_POINT))
        sh.run("mkdir -p %s/boot/efi" % self._ROOT_MOUNT_POINT)
        sh.run("/usr/sbin/busybox mount %s %s/boot/efi" % (self._bootPartition, self._ROOT_MOUNT_POINT))
        ...
        sh.run("/usr/sbin/busybox mount -o rbind /run %s/run" % self._ROOT_MOUNT_POINT)
        ...
        sh.run("/usr/sbin/busybox umount %s/run" % self._ROOT_MOUNT_POINT)
        sh.run("/usr/sbin/busybox umount %s/sys" % self._ROOT_MOUNT_POINT)
        sh.run("/usr/sbin/busybox umount %s/proc" % self._ROOT_MOUNT_POINT)
        sh.run("/usr/sbin/busybox umount %s/boot/efi" % self._ROOT_MOUNT_POINT)
